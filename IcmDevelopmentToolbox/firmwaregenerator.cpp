...
void FirmwareGenerator::s021ReturnPressed()
{
    ...
    {
        QMessageBox::warning(this, QString::fromLocal8Bit("警告"), QString::fromLocal8Bit("S0行数据无效"), QMessageBox::Yes);
        return;
    }
    ...
    QString versionQueryData = QInputDialog::getText(NULL,
                                                     QString::fromLocal8Bit("输入软件版本号"),
                                                     QString::fromLocal8Bit("请输入软件版本号, 需严格匹配格式: xx.xx.xx\n"),
                                                     QLineEdit::Normal,
                                                     "",
                                                     &isOK);
    //校验输入信息, 严格匹配xx.xx.xx
    ...
    {
        QMessageBox::warning(this, QString::fromLocal8Bit("警告"), QString::fromLocal8Bit("版本号数据有误, 未能更新版本号"), QMessageBox::Yes);
        return;
    }
}
...
void FirmwareGenerator::generateFirmwareForDiagnosis(void)
{
    ...
    else
    {
        ...
        QString crcAddressQueryData = QInputDialog::getText(NULL,
                                                         QString::fromLocal8Bit("输入crc校验码地址"),
                                                         QString::fromLocal8Bit("请输入crc校验码在芯片中的存储地址, 需由6位\n16进制字符组成(一般为FE8000或FEBE00)"),
                                                         QLineEdit::Normal,
                                                         "",
                                                         &isOK);
        //校验输入信息
        ...
        {
            QMessageBox::warning(this, QString::fromLocal8Bit("警告"), QString::fromLocal8Bit("输入的地址无效"), QMessageBox::Yes);
            return;
        }
    }

    QMessageBox::information(this, QString::fromLocal8Bit("固件信息"),
                             QString::fromLocal8Bit("零件号: ") + partNumber + QString::fromLocal8Bit(", 软件版本号: ") \
                             + softwareVersion + QString::fromLocal8Bit(", crc校验码存储地址: 0x") + s20cText.right(6),
                             QMessageBox::Yes);
...
void FirmwareGenerator::dealWithCalculateKeyCommand(void)
{
    ...
    QString seedQueryData = QInputDialog::getText(NULL,
                                                  QString::fromLocal8Bit("输入种子"),
                                                  QString::fromLocal8Bit("请输入由4位16进制字符组成的种子"),
                                                  QLineEdit::Normal,
                                                  "",
                                                  &isOK);
    ...
    if(isOK && regExp.exactMatch(seedQueryData))
    {
        ...
        if(!resultOK)
        {
            QMessageBox::warning(this, QString::fromLocal8Bit("警告"), QString::fromLocal8Bit("种子计算失败"), QMessageBox::Yes);
        }
        else
        {
            ...
            if(keyHexStr.size() > 4)
            {
                QMessageBox::warning(this, QString::fromLocal8Bit("警告"), QString::fromLocal8Bit("密钥长度不能超过4"), QMessageBox::Yes);
                return;
            }
            ...
            ptOutputWnd->appendPlainText(QString::fromLocal8Bit("   种子: [") + seedQueryData.toUpper() + QString::fromLocal8Bit("]  ->  密钥: [") + keyHexStr + "]");
            ptOutputWnd->appendPlainText("*************************************");
            ptOutputWnd->appendPlainText(QString::fromLocal8Bit("结果已经拷贝至系统剪贴板中:[2704") + keyHexStr + "]");
            ptOutputWnd->appendPlainText("*************************************");

            QMessageBox::information(this, QString::fromLocal8Bit("密钥计算结果"),
                                     QString::fromLocal8Bit("十六进制密钥计算结果为【") + keyHexStr + QString::fromLocal8Bit("】, 结果已经拷贝至系统剪贴板中"),
                                     QMessageBox::Yes);
        }
    }
    else
    {
        QMessageBox::warning(this, QString::fromLocal8Bit("警告"), QString::fromLocal8Bit("无效种子"), QMessageBox::Yes);
    }
}
...
void FirmwareGenerator::showHelpInfo(CmdType cmd)
{
    ...
    if(CMD_HELP == cmd)
    {
        hlpInfo << QString::fromLocal8Bit("《命令列表》");
        hlpInfo << QString::fromLocal8Bit("0 欢迎使用本软件，本页面介绍软件支持的指令，及其功能定义.");
        hlpInfo << QString::fromLocal8Bit("0.1 将<u>switch function</u>（或<u>input command</u>）切换至<u>run command</u>，"
                                          "在命令输入框键入<u>:help</u>或<u>:hlp</u>或<u>:?</u>后按回车键进入本页面.");
        hlpInfo << QString::fromLocal8Bit("0.2 其它命令的使用方法同0.1节，<u>指令输入不区分大小写</u>.");

        hlpInfo << QString::fromLocal8Bit("1 软件自身功能.");
        hlpInfo << QString::fromLocal8Bit("1.1 清屏.");
        hlpInfo << QString::fromLocal8Bit("1.1.1 定义：清空本软件屏幕当前正在显示的内容.");
        hlpInfo << QString::fromLocal8Bit("1.1.2 指令：<u>:clear screen</u>或<u>:cs</u>.");
        hlpInfo << QString::fromLocal8Bit("1.2 全屏显示.");
        hlpInfo << QString::fromLocal8Bit("1.2.1 定义：将软件切换至全屏显示.");
        hlpInfo << QString::fromLocal8Bit("1.2.2 指令：<u>:full screen</u>或<u>:fs</u>.");
        hlpInfo << QString::fromLocal8Bit("1.3 正常显示.");
        hlpInfo << QString::fromLocal8Bit("1.3.1 定义：将软件切换至正常大小显示.");
        hlpInfo << QString::fromLocal8Bit("1.3.2 指令：<u>:normal screen</u>或<u>:ns</u>.");
        hlpInfo << QString::fromLocal8Bit("1.4 保存配置.");
        hlpInfo << QString::fromLocal8Bit("1.4.1 定义：保存本软件的配置项，将在软件目录下生成config.json文件.");
        hlpInfo << QString::fromLocal8Bit("1.4.2 指令：<u>:save config file</u>或<u>:scf</u>.");
        hlpInfo << QString::fromLocal8Bit("1.4.3 备注：当前仅实现接口，尚未有实际需要存储的配置项.");
        hlpInfo << QString::fromLocal8Bit("1.5 加载配置.");
        hlpInfo << QString::fromLocal8Bit("1.5.1 定义：加载本软件的配置项，将加载软件目录下的config.json文件.");
        hlpInfo << QString::fromLocal8Bit("1.5.2 指令：<u>:load config file</u>或<u>:lcf</u>.");
        hlpInfo << QString::fromLocal8Bit("1.5.3 备注：当前仅实现接口，尚未有实际需要加载的配置项.");
        hlpInfo << QString::fromLocal8Bit("1.6 退出程序.");
        hlpInfo << QString::fromLocal8Bit("1.6.1 定义：退出本软件.");
        hlpInfo << QString::fromLocal8Bit("1.6.2 指令：<u>:quit</u>或<u>:q</u>.");

        hlpInfo << QString::fromLocal8Bit("2 Windows系统工具快捷开启功能.");
        hlpInfo << QString::fromLocal8Bit("2.1 运行Windows系统指令通运方法.");
        hlpInfo << QString::fromLocal8Bit("2.1.1 定义：像Windows系统自带的“运行”软件一样运行Windows系统指令.");
        hlpInfo << QString::fromLocal8Bit("2.1.2 指令：<u>::command</u>，其中<u>command</u>为Windows系统能识别的指令.");
        hlpInfo << QString::fromLocal8Bit("2.1.3 备注：受限于QT的运行机制，部分Windows系统指令可能不会被正确执行.");
        hlpInfo << QString::fromLocal8Bit("2.2 计算器.");
        hlpInfo << QString::fromLocal8Bit("2.2.1 定义：快速开启Windows系统自带的“计算器”软件.");
        hlpInfo << QString::fromLocal8Bit("2.2.2 指令：<u>:calculator</u>或<u>:calc</u>或<u>:c</u>.");
        hlpInfo << QString::fromLocal8Bit("2.3 任务管理器.");
        hlpInfo << QString::fromLocal8Bit("2.3.1 定义：快速开启Windows系统自带的“任务管理器”软件.");
        hlpInfo << QString::fromLocal8Bit("2.3.2 指令：<u>:taskmgr</u>或<u>:tm</u>.");
        hlpInfo << QString::fromLocal8Bit("2.4 画图.");
        hlpInfo << QString::fromLocal8Bit("2.4.1 定义：快速开启Windows系统自带的“画图”软件.");
        hlpInfo << QString::fromLocal8Bit("2.4.2 指令：<u>:mspaint</u>或<u>:mp</u>.");

        hlpInfo << QString::fromLocal8Bit("3 天有为开发工具功能.");
        hlpInfo << QString::fromLocal8Bit("3.1 bootloader合成.");
        hlpInfo << QString::fromLocal8Bit("3.1.1 定义：奇瑞M1系列bootloader合成方法介绍.");
        hlpInfo << QString::fromLocal8Bit("3.1.2 指令：<u>:bootloader?</u>或<u>:b?</u>.");
        ...
        hlpInfo << QString::fromLocal8Bit("3.4.3 备注：烧录此固件后会擦除仪表原来的app固件，需要重新烧录app固件.");
        hlpInfo << QString::fromLocal8Bit("3.5 生成M1系列boot code文件.");
        hlpInfo << QString::fromLocal8Bit("3.5.1 定义：生成M1系列boot代码段的.S19固件.");
        hlpInfo << QString::fromLocal8Bit("3.5.2 指令：<u>:m boot code</u>或<u>:mbc</u>.");
        hlpInfo << QString::fromLocal8Bit("3.5.3 备注：此固件不可单独烧录至仪表，需要合成到仪表app固件中，合成方法参考3.1节.");
        hlpInfo << QString::fromLocal8Bit("3.6 生成T1系列boot code文件.");
        hlpInfo << QString::fromLocal8Bit("3.6.1 定义：生成T1系列boot代码段的.S19固件.");
        hlpInfo << QString::fromLocal8Bit("3.6.2 指令：<u>:t boot code</u>或<u>:tbc</u>.");
        hlpInfo << QString::fromLocal8Bit("3.6.3 备注：此固件不可单独烧录至仪表，需要合成到仪表app固件中，合成方法参考3.1节.");
        hlpInfo << QString::fromLocal8Bit("3.7 生成S51EVFL的boot code文件.");
        ...
        hlpInfo << QString::fromLocal8Bit("4.1.2 指令：<u>:convert code to sQString::fromLocal8Biting</u>或<u>:c2s</u>.");
        hlpInfo << QString::fromLocal8Bit("4.2 根据种子计算密钥.");
        hlpInfo << QString::fromLocal8Bit("4.2.1 定义：某些诊断服务需要解锁，本功能根据输入的种子来计算对应的密钥.");
        hlpInfo << QString::fromLocal8Bit("4.2.2 指令：<u>:calculate key</u>或<u>:calc key</u>或<u>:ck</u>.");
        hlpInfo << QString::fromLocal8Bit("4.2.3 备注：输入<u>:calculate key?</u>或<u>:calc key?</u>或<u>:ck?</u>，显示安全解锁操作步骤帮助信息.");
        hlpInfo << QString::fromLocal8Bit("4.3 由Img2Lcd生成的位图C数组数据压缩工具.");
        hlpInfo << QString::fromLocal8Bit("4.3.1 定义：由Img2Lcd生成的位图C数组数据，将其中的全部0x00或0xff，按照出现的次数进行压缩.");
        hlpInfo << QString::fromLocal8Bit("4.3.2 指令：<u>:compress bmp</u>或<u>:cb</u>.");
    }
    else if(CMD_HELP_BOOTLOADER == cmd)
    {
        hlpInfo << QString::fromLocal8Bit("《BootLoader合成工具使用方法》");
        hlpInfo << QString::fromLocal8Bit("0 将<u>switch function</u>（或<u>input command</u>）切换至<u>add bootloader to firmware</u>.");
        hlpInfo << QString::fromLocal8Bit("1 加载bootloader代码段的两种方式.");
        hlpInfo << QString::fromLocal8Bit("1.1 方式一：加载默认bootloader代码段，先勾选<u>use default</u>，再更改其左侧的下拉选择框来选择加载特定型号的默认bootloader代码段.");
        hlpInfo << QString::fromLocal8Bit("1.2 方式二：加载其它bootloader代码段，去勾选<u>default</u>，点击<u>load bootloader</u>按钮选择其它bootloader文件.");
        hlpInfo << QString::fromLocal8Bit("2 点击<u>load file</u>按钮选择.S19原app文件.");
        hlpInfo << QString::fromLocal8Bit("3 点击<u>generate</u>按钮生成含bootloader的新app文件,并自动打开该文件所在的目录且选中该文件.");
    }
    else if(CMD_HELP_DIAGNOSIS == cmd)
    {
        hlpInfo << QString::fromLocal8Bit("《诊断仪app生成工具使用方法》");
        hlpInfo << QString::fromLocal8Bit("0 将<u>switch function</u>（或<u>input command</u>）切换至<u>gen firmware for diagnosis</u>.");
        hlpInfo << QString::fromLocal8Bit("1 点击<u>load file</u>按钮选择.S19原app文件.");
        hlpInfo << QString::fromLocal8Bit("2 输入S021数据，该数据最终将位于app的第一行.");
        hlpInfo << QString::fromLocal8Bit("2.1 在S021输入框输入<u>:t</u>并按回车键获取T19预置的S021数据；输入<u>:m</u>并按回车键获取m1afl2预置的S021数据；"
                      "输入<u>:s</u>并按回车键获取s51evfl预置的S021数据；输入<u>:a</u>并按回车键获取a13tev预置的S021数据.");
        hlpInfo << QString::fromLocal8Bit("2.2 正确输入S021数据后，将光标置于S021数据所在的输入框后，点击回车键可以修改版本号，版本号格式需严格匹配<u>xx.xx.xx</u>,x为0-9或a-f,字母不区分大小写，最终按大写字母写入文件.");
        hlpInfo << QString::fromLocal8Bit("2.3 将光标置于S021数据所在的输入框后，当S021输入框为空时按下回车键，会请求输入零件号，之后会请求输入软件版本信息，然后自动合成S021行数据，数据生成后依然可以使用2.1节的方法修改版本号.");
        hlpInfo << QString::fromLocal8Bit("3 点击<u>generate</u>按钮，生成诊断仪app文件,并自动打开该文件所在的目录且选中该文件.");
        hlpInfo << QString::fromLocal8Bit("4 该文件夹下还将自动生成flash driver文件，请将诊断仪app文件和flash driver文件一同加入压缩包提供给使用者.");
    }
    else if(CMD_HELP_DIAG_CALCULATE_KEY == cmd)
    {
        hlpInfo << QString::fromLocal8Bit("《安全解锁操作步骤》");
        hlpInfo << QString::fromLocal8Bit("0 在busmaster软件诊断窗口将[Send Tester Present]勾选为[ON]，激活3E服务.");
        hlpInfo << QString::fromLocal8Bit("1 在busmaster软件诊断窗口发送[10 03]，切换到扩展会话.");
        hlpInfo << QString::fromLocal8Bit("2 在busmaster软件诊断窗口发送[27 03]，请求计算安全密钥的种子.");
        hlpInfo << QString::fromLocal8Bit("3 将busmaster软件诊断窗口收到的[67 03 xx xx]中的后两个字节即[xx xx]输入到弹出的[seed query]窗口中.");
        hlpInfo << QString::fromLocal8Bit("4 点击[seed query]窗口的[OK]将会返回计算好的安全密钥.");
        hlpInfo << QString::fromLocal8Bit("5 输入密钥.");
        hlpInfo << QString::fromLocal8Bit("5.1 方法一：在busmaster软件诊断窗口发送[27 04 hh hh]，其中[hh hh]为第4步所得的数值.");
        hlpInfo << QString::fromLocal8Bit("5.2 方法二：第4步完成后，程序已经将结果复制到系统剪贴板中，在busmaster软件诊断发送窗口，使用鼠标右键单击并选择粘贴，然后发送即可.");
    }
...
{
    ptOutputWnd->clear();
    ptOutputWnd->appendPlainText(QString::fromLocal8Bit("欢迎使用文件转字符串工具"));
...
{
    ptOutputWnd->clear();
    ptOutputWnd->appendPlainText(QString::fromLocal8Bit("欢迎使用Image2Lcd生成的C数组的数组压缩工具"));

    const int DEVIDE_AMOUNT = 255; //下位机存储宽和高各用一个字节，此处应确保此值不大于255（即0xff）
    QString filePathName = QFileDialog::getOpenFileName(this, QString::fromLocal8Bit("请选择由Image2Lcd生成的位图数组文件"),
                                                        "",
                                                        tr("C files (*.c *.h)"));
    QString filePath = QFileInfo(filePathName).absolutePath();
    ...
    if(filePathName.isEmpty())
    {
        QMessageBox::warning(this, QString::fromLocal8Bit("警告"), QString::fromLocal8Bit("未选择文件, 压缩失败"), QMessageBox::Yes);
        return;
    }
    ...
    if(!targetFile.open(QIODevice::ReadOnly | QIODevice::Text))
    {
        QMessageBox::warning(this, QString::fromLocal8Bit("警告"), QString::fromLocal8Bit("无法打开") + filePathName, QMessageBox::Yes);
        return;
    }
