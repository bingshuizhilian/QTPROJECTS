...
void MainWindow::showHelpInfo(CmdType cmd)
{
    ...
    else if(CMD_HELP_DIAG_CALCULATE_KEY == cmd)
    {
        ...
        hlpInfo << tr("5 输入密钥.");
        hlpInfo << tr("5.1 方法一：在busmaster软件诊断窗口发送[27 04 hh hh]，其中[hh hh]为第3步所得的数值.");
        hlpInfo << tr("5.2 方法二：第3步完成后，程序已经将结果复制到系统剪贴板中，在busmaster软件诊断发送窗口，使用鼠标右键单击并选择粘贴，然后发送即可.");
    }
